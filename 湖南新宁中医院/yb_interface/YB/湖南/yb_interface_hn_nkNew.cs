...
ywCode);
        ...
        public static object[] YBMZJSD(object[] objParam)
        {
            ...
            #region 新sql
            strSql = @"select a.jzlsh,a.xm, case when b.xb='1' then '男' else '女' end xb,'{1}' as yljgbh,y.bzmem8 as yljgdj,y.bzname as yljgmc,
                                    ...
                                    b.ghdjsj as ryrq,
                                    a.cyrq as cyrq,
									isnull((select b5sfnm from bz05h where b5sfno=cf.sflb),'其他') as xmmc
                                    ,sum(cf.je) as xmje, 
									sum((case cf.sfxmdj when '01' then isnull(cf.fhzcfwje,0.00) else 0.00 end)) as xmjlje,
									sum((case cf.sfxmdj when '02' then isnull(cf.fhzcfwje,0.00) else 0.00 end)) as xmylje,
									sum((case cf.sfxmdj when '03' then isnull(cf.fhzcfwje,0.00) else 0.00 end)) as  xmzfje ,
                                   ...
                                   '' as zyts,
									...
									(select bzname from bztbd where bzcodn='hi_type' and bzkeyx=c1.xzlx ) as xz, e.bzname as rysflb, --
									b.ksmc as kb,'' as zych,
                                    a.grbh,z.bzname as yllb,c1.DWMC dwmc,
                                    e.bzname as zlzt,
									...
									a.jslsh as jsid,h.m1telp as tel,a.gwybz,'0' as yljzjjzf,(select i.DQBH from YBICKXX i where i.grbh=a.grbh) as tcqh,a.zbxje,c1.SJNL as nl
                                    from ybfyjsdr a
                                    left join ybmzzydjdr b on a.jzlsh=b.jzlsh and a.ybjzlsh=b.ybjzlsh and a.cxbz=b.cxbz
                                    left join ybrycbxx c on a.grbh=c.grbh   and insutype=a.xzlx
                                   ...
                                   where  a.cxbz=1 and a.jzlsh='{0}' and a.djhin='{2}'
									group by a.jzlsh,a.xm,b.xb, y.bzmem8,y.bzname ,c1.GMSFHM,jssj
									,c1.DWMC,c1.CSRQ,ryrq, cyrq,cf.sflb,b.ghdjsj,a.cyrq
									,rylb,rysflb,b.ksmc,s.psn_idet_type,c1.xzlx,c.psn_type,
                                    a.grbh,d.bzname ,c1.dwmc,e.bzname,
									...
									a.jbylbxtcjjzfbl,h.m1telp ,a.gwybz,z.bzname,a.zbxje,c1.SJNL
									union all 
									 select top 1 a.jzlsh,a.xm, case when b.xb='1' then '男' else '女' end xb,'{1}' as yljgbh,y.bzmem8 as yljgdj,y.bzname as yljgmc,
									isnull(c1.GMSFHM,'') as  idcard,a.sysdate as jssj,c1.DWMC as dwmc, c1.CSRQ as csrq,--convert(varchar,convert(date,substring(c1.CSRQ,0,9)))
                                    b.ghdjsj as ryrq,
                                    a.cyrq as cyrq,
									'合计' as xmmc,
                                   (select sum(isnull(je,0.00)) from ybcfmxscfhdr where jzlsh='{0}') as xmje, 
									(select sum(isnull(fhzcfwje,0.00)) from ybcfmxscfhdr where jzlsh='{0}') as xmjlje,
									(select sum(isnull(xxzfje,0.00)) from ybcfmxscfhdr where jzlsh='{0}') as xmylje,
									(select sum(isnull(zfje,0.00)) from ybcfmxscfhdr where jzlsh='{0}') as xmzfje ,
									(select sum(isnull(cxjzfje,0.00)) from ybcfmxscfhdr where jzlsh='{0}') as xmqtje,
                                   '' as zyts,
									(case c.psn_type when '11' then '在职' when '21' then '退休' when '31' then '离休'when '32' then '老红军'
									when '41' then '学龄前儿童' when '42' then '中小学生'when '43' then '大学生'when '50' then '成年居民' 
									else '' end) as rylb,
									(select bzname from bztbd where bzcodn='hi_type' and bzkeyx=a.xzlx ) as xz, e.bzname as rysflb, 
									b.ksmc as kb,'' as zych,
                                    a.grbh,z.bzname as yllb,c1.DWMC dwmc,
                                    e.bzname as zlzt,
                                    a.ybjzlsh,a.ylfze,a.qfxbz,a.bcfhyb,isnull(a.cxjfy,0)+isnull(a.zffy,0) zffy,a.xxzfje,zlfy,tcjjzf, a.xjzf,a.zhzf,a.dejjzf,a.mzjzfy,a.bcsydbzffy,a.bcbcbxzffy,a.dbjjzf,a.qtjjzf as qybcylzf,a.qybcylbxjjzf,a.cxjfy,a.yyfdfy,
                                    ...
                                    left join mz01h h on h.m1ghno=a.jzlsh 
                                   where  a.cxbz=1 and a.jzlsh='{0}' and a.djhin='{2}'	";
            strSql = string.Format(strSql, jzlsh, YBJGBH, djh);
            #endregion
            ...
            string nddbbxzf = "";
            string ndyljzzf = "";
            object[] objquery = YBRYLJXJCX(new object[] { grbh, ljny });
            ...
            cfile = Application.StartupPath + @"\FastReport\YB\湖南新宁县中医医院国家医保门诊结算单.frx";
            sfile = spath + @"\YB\湖南新宁县中医医院国家医保门诊结算单.frx";
            oldNew = "new";
            A4 = "A";
            ...
            report.SetParameterValue("ndzyts", ndzycs);
            report.SetParameterValue("ndfylj", ndfy);
            report.SetParameterValue("ndylfze", ylfhj);
            report.SetParameterValue("ndqfxje", ndqfx);
            report.SetParameterValue("ndtczf", ndtczf);
            report.SetParameterValue("ndzczf", ndzczf);
            report.SetParameterValue("ndzcZf", ndzcpay);
            report.SetParameterValue("nddejjzf", nddejjzf);
            report.SetParameterValue("dbbxhgfy", nddbzf);
            report.SetParameterValue("dbbxzf", nddbbxzf);
            report.SetParameterValue("yljzzf", ndyljzzf);
            report.Show();
            report.Dispose();
        ...
        public static object[] YBZYFYQDDY(object[] objParam)
        {
            ...
            CONVERT(decimal(10,2),ROUND((f.fhzcfwje/f.je),2))  as bxbl,
            h.b5sfnm
            from ybcfmxscfhdr f
