...
using Item = yb_interfaces.YB.湖南.Config.Item;

namespace yb_interfaces
{
    public static class Functions
    {
        public static List< Item> getXmlConfig(string path)
        {
            ...
            try
            {
                ...
                foreach (XElement item2 in xElement.Elements("appSettings").Elements("add"))
                {
                    ...
                    if (item.TableName.Equals("IDCARD"))
                    {
                        item.IdCard = item2.Attribute("value").Value;
                    }
                    if (item.TableName.Equals("JKBBH"))
                    {
                        item.jkVersion = item2.Attribute("value").Value;
                    }
                    if (item.TableName.Equals("DKURL"))
                    {
                        item.DkUrl = item2.Attribute("value").Value;
                    }
