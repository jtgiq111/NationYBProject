...
{
    public partial class Frm_ybcxYT : InfoForm
    {
        public Frm_ybcxYT()
        {
            InitializeComponent();
        ...
        private void frm_ybcxSR_Load(object sender, EventArgs e)
        {
            ...
            string strSql = string.Format(@"select distinct a.jzlsh,a.xm, a.grbh, a.yldylb,a.ksmc,a.cwh,a.ysxm,
                                            ...
                                            c.djhin as djh,c.sysdate from ybmzzydjdr a
                                            left join ybcfmxscfhdr b on a.jzlsh=b.jzlsh and b.cxbz=1
...
//            alvWHERE_MZ.SetDataSource(ds_MZ.Copy());
        ...
        private void alvWHERE_AfterConfirm(object sender, EventArgs e)
        {
            if (alvWHERE.currentDataRow != null)
            {
                ...
                #endregion
                #region 获取医保上传总费用
                strSql = string.Format(@"select isnull(SUM(je),0.00) as ybfy from ybcfmxscfhdr where jzlsh = '{0}' and cxbz=1", txtJZLSH_ZY.Text);
                ds.Tables.Clear();
