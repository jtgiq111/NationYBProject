...
{
    public partial class Form3 : InfoForm
    {
        #region 变量
        ...
        internal void databindData()
        {
            ...
            switch (hislx)
            {
                case "1": //药品
                    ...
                    left join ybhisdzdr c on a.y1ypno=c.hisxmbh
                ...
                case "3": //诊疗项目
                    strSql = string.Format(@"with zlxm as(select a.b5item as hisxmbm,a.b5name as hisxmmc,'' as hisgg,'' as hisjx,a.b5pymx as pym,a.b5unit as dw,1 as sl,a.b5sfam as je,
                                            ...
                                            left join ybhisdzdr c on a.hisxmbm=c.hisxmbh) select * from zlxm1 a  where 1=1 ");
                    if (!string.IsNullOrEmpty(xmmc))
                    {
        ...
        internal void 狗屁需求()
        {
            ...
            switch (hislx)
            {
                ...
                case "3": //诊疗项目    
                    ...
                    left join ybhisdzdr c on a.b5item=c.hisxmbh");
                    break;
                case "4": //其他
                    ...
                    left join ybhisdzdr c on a.b5item=c.hisxmbh where 1!=1");
                    break;
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string strSql = string.Format(@"delete from ybhisdzdr where hisxmbh='{0}'", yyxmbh);
            liSql.Add(strSql);
            strSql = string.Format(@"insert into ybhisdzdr(hisxmbh,hisxmmc,ybxmbh,ybxmmc,xmnh,cwnn,dw,sm) values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}')",
                                    yyxmbh, yyxmmc, ybxmbm, ybxmmc, xmnh, cwnn, dw, sm);
            liSql.Add(strSql);
