...
{
    public partial class Frm_kfryybzyfyscdr : InfoForm
    {
        string xmbhs = "";

        public Frm_kfryybzyfyscdr()
        {
            InitializeComponent();
        }
        ...
        private void txt_zyh_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    from zy01h join ybmzzydjdr on z1zyno = jzlsh and cxbz = 1
                    join zy03d on z1zyno = z3zyno and z1ghno = z3ghno and z1comp = z3comp 
                    where left(z1endv ,1) = '0' and left(z3kind, 1) in ('2', '4')
                    and isnull(z3ybup, '') = '' and isnull(z3fphx, '') = '' and z3zyno = '{0}'
                    group by z1zyno, z1hznm, z1ksno, z1ksnm, z1amt1, z1amt2, z1amt3, xm, grbh, jylsh, kh 
                    having sum(case left(z3endv, 1) when '4' then -z3jzcs else z3jzcs end) > 0 and sum(case left(z3endv, 1) when '4' then -z3jzje else z3jzje end) > 0", zyh);
                    DataSet ds = CliUtils.ExecuteSql("shs01", "cmd", sql, CliUtils.fLoginDB, true, CliUtils.fCurrentProject);
                    infoDataGridView1.DataSource = ds.Tables[0];
                }
            }
        ...
        private void btn_cltf2018_Click(object sender, EventArgs e)
        {
            ...
            obj = yb_interface.ybs_interface("4320",obj);
            obj = new object[] { jzlsh, xmbhs };
            obj = yb_interface.ybs_interface("5012",obj);
