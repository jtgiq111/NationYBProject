...
{
    public partial class Frm_ybbbrydr : InfoForm
    {
        public Frm_ybbbrydr()
        {
            InitializeComponent();
        ...
        private void Fybbbry_Load(object sender, EventArgs e)
        {
            ...
            txt_yllb.setWhere = "bzcodn = 'YL' and bzmem3 like '%Z%' and (bzpymx like '%{0}%' or bzwbmx like '%{0}%' or bzname like '%{0}%' or bzmem1 like '%{0}%')";
        ...
        private bool GetYBInfo(ref string backmsg)
        {
            ...
            if (back_mess[1].ToString() == "1")
            {
                ...
                txt_tcqh.Text = strP[11];
                sql = string.Format("select bzname from bztbd(nolock) where bzcodn = 'DQ' and bzkeyx = '{0}' and bzusex = 1", strP[11]);
                dt = CliUtils.ExecuteSql("sybdj", "cmd", sql, CliUtils.fLoginDB, true, CliUtils.fCurrentProject).Tables[0];

                if (dt != null && dt.Rows.Count > 0)
                {
                    DataRow dr = dt.Rows[0];
                    txt_tcdq.Text = dr["bzname"].ToString();
                }
