...
namespace ybinterface_lib
{
    public partial class FrmTotalDZ : InfoForm
    {
        ...
        public int InitDs()
        {
            ...
            dsres.Tables[0].Columns.Add("对账差额", typeof(string));
        ...
        private void btnSel_Click(object sender, EventArgs e)
        {
            ...
            {
                #region 汇总绑定值
                string StrSql = @"select sum(isnull(y.ylfze,0)) as ylfze,sum(isnull(y.tcjjzf,0)) as tczfje,sum(isnull(y.xjzf,0)) as xjzfje from  ybfyjsdr y where y.sysdate>='{0}' and (y.jbr='{2}' or 'ALL'='{2}')  and y.sysdate<'{1}' and y.isgs='1'";
                StrSql = string.Format(StrSql, beginTime, EndTime, CliUtils.fLoginUser);
                ...
                //医保返回
                (int code, string result) = yb_interface_sy_zrNew.TotalDZ(start.ToString("yyyyMMddHHmmss"), end.ToString("yyyyMMddHHmmss"), ref objlist);
                if (code <= 0)
                {
            ...
            else if (tabIndex == 1)
            {
                ...
                StrSqlmx = string.Format(StrSqlmx, beginTime, EndTime, CliUtils.fLoginUser);
