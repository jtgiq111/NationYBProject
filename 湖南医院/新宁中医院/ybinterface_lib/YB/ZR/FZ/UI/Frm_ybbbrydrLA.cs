...
{
    public partial class Frm_ybbbrydrLA : InfoForm
    {
        public Frm_ybbbrydrLA()
        {
            InitializeComponent();
        }
        ...
        private void txt_zyno_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                string strSql = string.Format(@"select * from zy03dw where z3zyno='{0}'and z3endv like '1%'", txt_zyno.Text.Trim());
                DataSet ds = CliUtils.ExecuteSql("sybdj", "cmd", strSql, CliUtils.fLoginDB, true, CliUtils.fCurrentProject);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    MessageBox.Show("患者已经出院结算，不能进行医保操作！");
                    return;
                }
        ...
        private void Fybbbry_Load(object sender, EventArgs e)
        {
            ...
            string strSQL = string.Format(@"select CODE,NAME,PYM,WB from YBXMLBZD where lbmc='生育类别'");
            DataSet ds = CliUtils.ExecuteSql("sybdj", "cmd", strSQL, CliUtils.fLoginDB, true, CliUtils.fCurrentProject);
            alv_sylb.SetDataSource(ds.Copy());
            txtSYLB.Tag = "";
            #endregion
            ...
            alv_jhsylb.SetDataSource(ds1.Copy());
            txtJHSYLB.Tag = "";
            #endregion
        }
        ...
        private void btn_RegisterYB_Click(object sender, EventArgs e)
        {
            ...
            object[] back_mess, param = new object[] { txt_zyno.Text, txt_yllb.Tag, txt_bz.Tag, txt_bz.Text, txt_grbh.Tag, txt_lyjg.Tag, txt_lyjg.Text, txt_yllb.Text, txt_dgys.Tag, txt_dgys.Text, txtZSZH.Text.Trim(), txtSYLB.Tag, txtJHSYLB.Tag };
